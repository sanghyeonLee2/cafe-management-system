...
router.post("/", async (req, res, next) => {
    ...
    const {menuOrderPayment, userId, menuOrderTotalPrice} = req.body.menuOrder;
    ...
    try {
        ...
        for (const element of orderList) {
            ...
            await Material.update(
                {
                    materialQuantity: Sequelize.literal(`material_quantity - ${element.orderDetailQuantity}`)
                },
                {
                    where: {materialNum},
                    transaction: t
    ...
    try {
        const query =
            `SELECT * FROM order_detail a INNER JOIN menu_item b ON 
