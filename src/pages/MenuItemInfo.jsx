...
function MenuItemInfo(props) {
    ...
    const directOrder = async () => {
        ...
        const result = await axios.post("http://localhost:8080/menu-order", menuOrderObj)
    ...
    useEffect(() => {
        const fetchMenuInfo = async () => {
            const result = await
                axios.get("http://localhost:8080/menu-item/recipe", {params: {menuItemNum: menuItemInfo.menuItemNum}})
            if (result.status === 200) {
    ...
    return (
        <div>
            {directOrderPopUp && <div>
                ...
                <button type={"button"} onClick={directOrder}>완료</button>
            </div>}
            <h3>메뉴 상세 정보</h3>
            <div>
                <ul>
                    <li>메뉴번호: {menuItemInfo.menuItemNum}</li>
                    <li>특별 메뉴 여부: {menuItemInfo.menuItemIsSpecialMenu === 1 ? "O" : "X"}</li>
