        ...
        <div>
            ...
            <div>
                <form onSubmit={supplyOnsubmit}>
                    <h3>재료 공급</h3>
                    납기<input type={"date"} required onChange={(e) => setSupplyPeriodPayment(e.target.value)}/>
                    {[...Array(materialArr.length)].map((val, idx) => {
                        return (<div key={idx}>
                            재료이름<input type={"text"} required name={"materialName"} onChange={(e) => formInputChg(e, idx)}/>
                            개수<input type={"number"}  required name={"materialQuantity"} onChange={(e) => formInputChg(e, idx)}/>
                            재료단위<input type={"text"} required name={"materialUnit"} onChange={(e) => formInputChg(e, idx)}/>
                        </div>)
                    })}
