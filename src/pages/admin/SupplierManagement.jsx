...
function SupplierManagement(props) {
    ...
    const insertSupplierBtn = async () => {
        const result = await axios.post("http://localhost:8080/supplier/", supplierInfo)
        if (result.status === 200) {
            alert("공급업체가 추가되었습니다.")
            setTrigger(!trigger);
            return
    ...
    const udtSupplierForm = (ele) => {
        setSupplierInfo({supplierName: `${ele.supplierName}`, supplierAddress: `${ele.supplierAddress}`})
        setIsPopupOpen(ele.supplierNum)
    }

    const udtSupplierBtn = async () => {
        const result = await axios.put("http://localhost:8080/supplier/", {supplierNum: isPopupOpen, supplierInfo})
        if (result.status === 200) {
            alert("공급업체가 수정되었습니다.")
    ...
    const delSupplierBtn = async (supplierNum) => {
        const result = await axios.delete("http://localhost:8080/supplier/", {data: {supplierNum}})
        if (result.status === 200) {
            alert("공급업체가 삭제되었습니다.")
            setTrigger(!trigger);
            return
        ...
        <div>
            {isPopupOpen &&
                <div>
                    ...
                    <input type={"text"} defaultValue={supplierInfo.supplierName} name={"supplierName"}
                           onChange={handleInputChg}/>
                    <input type={"text"} defaultValue={supplierInfo.supplierAddress} name={"supplierAddress"}
                           onChange={handleInputChg}/>
                    ...
                    showSupplierInfo.map((ele) => {
                        return (
                            <tr key={ele.supplierNum}>
                                <td><Link to={"/admin/supplier-management/supplier/"} state={ele}>
                                    {ele.supplierNum}
                                </Link></td>
                                ...
                                <td>
                                    <button type={"button"}
                                            onClick={() =>udtSupplierForm(ele)}>수정
                                    </button>
                                    <button type={"button"} onClick={() => delSupplierBtn(ele.supplierNum)}>삭제</button>
                                </td>
                            </tr>
                        )
                    })}
                <tr>
                    <td><input type={"text"} placeholder={"업체번호 자동생성"} disabled/></td>
                    <td><input type={"text"} name={"supplierName"}
                               onChange={handleInputChg}/></td>
