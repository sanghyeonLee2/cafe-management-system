...
function MaterialManagement(props) {
    ...
    const inputFormChg = (e) => {
        setMaterialFormState((prev) => ({
            ...prev, [e.target.name]: e.target.value, materialNum: popUp.materialNum
        }))
    }

    const materialUdtBtn = async () => {
        const result = await axios.put("http://localhost:8080/material/", materialFormState)
        if (result.status === 200) {
            alert("재료수정 성공")
            setTrigger(!trigger)
            return;
    ...
    return (
        <>
            {popUp && <MaterialPageLayout>
                <h3>재료번호 : {popUp.materialNum}</h3>
                재료단위 : <input type={"text"} name={"materialUnit"} onChange={inputFormChg}
                              defaultValue={popUp.materialUnit}/>
                이름 : <input type={"text"} name={"materialName"} defaultValue={popUp.materialName}
                            onChange={inputFormChg}/>
                수량 : <input type={"number"} name={"materialQuantity"} defaultValue={popUp.materialQuantity}
                            onChange={inputFormChg}/>
                <button type={"button"} onClick={() => materialUdtBtn()}>완료</button>
                <button type={"button"} onClick={() => setPopUp(false)}>취소</button>
            </MaterialPageLayout>}
            <CenterLayout>
                <table border={"1"}>
                    ...
                    {showMaterial.length > 0 &&
                        showMaterial.map((ele) =>
                            <tr key={ele.materialNum}>
                                ...
                                <td>
                                    <button type={"button"} onClick={() => popUpOpen(ele)}>수정</button>
                                </td>
                            </tr>)
                    }
                    </tbody>
